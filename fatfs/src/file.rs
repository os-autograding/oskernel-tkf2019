...
pub struct File<IO: ReadWriteSeek, TP, OCC> {
...
impl<'a, IO: ReadWriteSeek, TP, OCC> File<IO, TP, OCC> {
    pub fn new(
    ...
    pub fn truncate(&mut self) -> Result<(), Error<IO::Error>> {
        ...
        if let Some(current_cluster) = self.current_cluster {
            ...
            self.fs.clone().truncate_cluster_chain(current_cluster)
        } else {
            debug_assert!(self.offset == 0);
    ...
    pub fn extents(&mut self) -> impl Iterator<Item=Result<Extent, Error<IO::Error>>>{
        ...
        Some(core::iter::once(Ok(first)).chain(fs.clone().cluster_iter(first))
    ...
    pub fn abs_pos(&self) -> Option<u64> {
...
impl<IO: ReadWriteSeek, TP: TimeProvider, OCC> File<IO, TP, OCC> {
...
impl<IO: ReadWriteSeek, TP, OCC> Drop for File<IO, TP, OCC> {
    fn drop(&mut self) {
...
impl<IO: ReadWriteSeek, TP, OCC> Clone for File<IO, TP, OCC> {
    fn clone(&self) -> Self {
        File {
...
impl<IO: ReadWriteSeek, TP, OCC> IoBase for File<IO, TP, OCC> {
    type Error = Error<IO::Error>;
}

impl<IO: ReadWriteSeek, TP: TimeProvider, OCC> Read for File<IO, TP, OCC> {
    fn read(&mut self, buf: &mut [u8]) -> Result<usize, Self::Error> {
        ...
        let current_cluster_opt = if self.offset % cluster_size == 0 {
            ...
            match self.current_cluster {
                ...
                Some(n) => {
                    let r = self.fs.clone().cluster_iter(n).next();
                    match r {
...
impl<IO: ReadWriteSeek, TP: TimeProvider, OCC> Write for File<IO, TP, OCC> {
    fn write(&mut self, buf: &[u8]) -> Result<usize, Self::Error> {
        ...
        let current_cluster = if self.offset % cluster_size == 0 {
            ...
            if let Some(n) = next_cluster {
                ...
                let new_cluster = self.fs.clone().alloc_cluster(self.current_cluster, self.is_dir())?;
...
impl<IO: ReadWriteSeek, TP, OCC> Seek for File<IO, TP, OCC> {
    fn seek(&mut self, pos: SeekFrom) -> Result<u64, Self::Error> {
        ...
        let new_cluster = if new_offset == 0 {
            ...
            let mut iter = self.fs.clone().cluster_iter(first_cluster);
