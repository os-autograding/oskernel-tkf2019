...
pub struct DirFileEntryData {
    ...
    pub reserved_0: u8,
    pub create_time_0: u8,
    pub create_time_1: u16,
    pub create_date: u16,
    pub access_date: u16,
...
impl DirFileEntryData {
    pub fn new(name: [u8; SFN_SIZE], attrs: FileAttributes) -> Self {
        Self {
            name,
            attrs,
            ..Self::default()
    ...
    pub fn first_cluster(&self, fat_type: FatType) -> Option<u32> {
...
impl DirEntryData {
    pub fn serialize<E: IoError, W: Write<Error = Error<E>>>(&self, wrt: &mut W) -> Result<(), Error<E>> {
        match self {
    ...
    pub fn deserialize<E: IoError, R: Read<Error = Error<E>>>(rdr: &mut R) -> Result<Self, Error<E>> {
...
impl DirEntryEditor {
    ...
    pub fn inner(&self) -> &DirFileEntryData {
        &self.data
    }

    pub fn set_first_cluster(&mut self, first_cluster: Option<u32>, fat_type: FatType) {
    ...
    pub fn flush<IO: ReadWriteSeek, TP, OCC>(&mut self, fs: Rc<FileSystem<IO, TP, OCC>>) -> Result<(), IO::Error> {
        if self.dirty {
            self.write(fs)?;
            self.dirty = false;
        }
    ...
    fn write<IO: ReadWriteSeek, TP, OCC>(&self, fs: Rc<FileSystem<IO, TP, OCC>>) -> Result<(), IO::Error> {
...
#[derive(Clone)]
pub struct DirEntry<IO: ReadWriteSeek, TP, OCC> {
    pub data: DirFileEntryData,
...
impl<IO: ReadWriteSeek, TP, OCC: OemCpConverter> DirEntry<IO, TP, OCC> {
    ...
    pub fn is_same_entry(&self, other: &DirEntry<IO, TP, OCC>) -> bool {
    ...
    pub fn raw_short_name(&self) -> &[u8; SFN_SIZE] {
        &self.data.name
    }
...
impl<IO: ReadWriteSeek, TP, OCC> fmt::Debug for DirEntry<IO, TP, OCC> {
