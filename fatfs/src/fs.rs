...
pub struct FsOptions<TP, OCC> {
    ...
    pub oem_cp_converter: OCC,
    pub time_provider: TP,
}
...
pub struct FileSystem<IO: ReadWriteSeek, TP, OCC> {
    pub disk: RefCell<IO>,
    pub options: FsOptions<TP, OCC>,
    pub fat_type: FatType,
    pub bpb: BiosParameterBlock,
    pub first_data_sector: u32,
    pub root_dir_sectors: u32,
    pub total_clusters: u32,
    pub fs_info: RefCell<FsInfoSector>,
    pub current_status_flags: Cell<FsStatusFlags>,
}
...
impl<IO: Read + Write + Seek, TP, OCC> FileSystem<IO, TP, OCC> {
    ...
    pub fn bytes_from_clusters(&self, clusters: u32) -> u64 {
        self.bpb.bytes_from_sectors(self.bpb.sectors_from_clusters(clusters))
    }

    pub fn clusters_from_bytes(&self, bytes: u64) -> u32 {
        self.bpb.clusters_from_bytes(bytes)
    }

    fn fat_slice(self: Rc<Self>) -> impl ReadWriteSeek<Error = Error<IO::Error>> {
    ...
    pub fn cluster_iter(
        ...
        let disk_slice = self.clone().fat_slice();
        ClusterIterator::new(disk_slice, self.fat_type, cluster)
    }

    pub fn truncate_cluster_chain(self: Rc<Self>, cluster: u32) -> Result<(), Error<IO::Error>> {
        let mut iter = self.clone().cluster_iter(cluster);
    ...
    pub fn free_cluster_chain(self: Rc<Self>, cluster: u32) -> Result<(), Error<IO::Error>> {
        let mut iter = self.clone().cluster_iter(cluster);
    ...
    pub fn alloc_cluster(self: Rc<Self>, prev_cluster: Option<u32>, zero: bool) -> Result<u32, Error<IO::Error>> {
        let hint = self.fs_info.borrow().next_free_cluster;
        let cluster = {
            let mut fat = self.clone().fat_slice();
    ...
    pub fn read_status_flags(self: Rc<Self>) -> Result<FsStatusFlags, Error<IO::Error>> {
        let bpb_status = self.clone().bpb.status_flags();
        let fat_status = read_fat_flags(&mut self.clone().fat_slice(), self.fat_type)?;
        Ok(FsStatusFlags {
    ...
    fn recalc_free_clusters(self: Rc<Self>) -> Result<u32, Error<IO::Error>> {
        let mut fat = self.clone().fat_slice();
    ...
    pub fn set_dirty_flag(&self, dirty: bool) -> Result<(), IO::Error> {
    ...
    pub fn root_dir(self: Rc<Self>) -> Dir<IO, TP, OCC> {
        let root_rdr = {
            match self.fat_type {
                ...
                FatType::Fat32 => DirRawStream::File(File::new(Some(self.bpb.root_dir_first_cluster), None, self.clone())),
            }
        };
...
impl<IO: ReadWriteSeek, TP: TimeProvider, OCC: OemCpConverter> FileSystem<IO, TP, OCC> {
    ...
    /// `Error::Io` will be returned if the underlying storage object returned an I/O error.
    #[cfg(feature = "alloc")]
    pub fn read_volume_label_from_root_dir(self: Rc<Self>) -> Result<Option<String>, Error<IO::Error>> {
        // Note: DirEntry::file_short_name() cannot be used because it interprets name as 8.3
        // (adds dot before an extension)
        let volume_label_opt = self.clone().read_volume_label_from_root_dir_as_bytes()?;
    ...
    ///
    /// `Error::Io` will be returned if the underlying storage object returned an I/O error.
    pub fn read_volume_label_from_root_dir_as_bytes(self: Rc<Self>) -> Result<Option<[u8; SFN_SIZE]>, Error<IO::Error>> {
...
pub struct FsIoAdapter<IO: ReadWriteSeek, TP, OCC> {
    pub fs: Rc<FileSystem<IO, TP, OCC>>,
}

impl<IO: ReadWriteSeek, TP, OCC> IoBase for FsIoAdapter< IO, TP, OCC> {
    type Error = IO::Error;
}

impl<IO: ReadWriteSeek, TP, OCC> Read for FsIoAdapter<IO, TP, OCC> {
    fn read(&mut self, buf: &mut [u8]) -> Result<usize, Self::Error> {
        self.fs.disk.borrow_mut().read(buf)
    }
}

impl<IO: ReadWriteSeek, TP, OCC> Write for FsIoAdapter<IO, TP, OCC> {
...
impl<IO: ReadWriteSeek, TP, OCC> Seek for FsIoAdapter<IO, TP, OCC> {
...
impl<IO: ReadWriteSeek, TP, OCC> Clone for FsIoAdapter<IO, TP, OCC> {
    fn clone(&self) -> Self {
...
pub struct FormatVolumeOptions {
    ...
    pub max_root_dir_entries: Option<u16>,
    pub fats: Option<u8>,
    pub media: Option<u8>,
    ...
    pub volume_label: Option<[u8; SFN_SIZE]>,
}
