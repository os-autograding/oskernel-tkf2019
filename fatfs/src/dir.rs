...
pub enum DirRawStream<IO: ReadWriteSeek, TP, OCC> {
    File(File<IO, TP, OCC>),
    Root(DiskSlice<FsIoAdapter<IO, TP, OCC>, FsIoAdapter<IO, TP, OCC>>),
}

impl<IO: ReadWriteSeek, TP, OCC> DirRawStream<IO, TP, OCC> {
...
impl<IO: ReadWriteSeek, TP, OCC> Clone for DirRawStream<IO, TP, OCC> {
    fn clone(&self) -> Self {
        match self {
...
impl<IO: ReadWriteSeek, TP, OCC> IoBase for DirRawStream<IO, TP, OCC> {
    type Error = Error<IO::Error>;
}

impl<IO: ReadWriteSeek, TP: TimeProvider, OCC> Read for DirRawStream<IO, TP, OCC> {
...
impl<IO: ReadWriteSeek, TP: TimeProvider, OCC> Write for DirRawStream<IO, TP, OCC> {
...
impl<IO: ReadWriteSeek, TP, OCC> Seek for DirRawStream<IO, TP, OCC> {
...
enum DirEntryOrShortName<IO: ReadWriteSeek, TP, OCC> {
...
pub struct Dir<IO: ReadWriteSeek, TP, OCC> {
    stream: DirRawStream<IO, TP, OCC>,
    fs: Rc<FileSystem<IO, TP, OCC>>,
}

impl<'a, IO: ReadWriteSeek, TP, OCC> Dir<IO, TP, OCC> {
    pub fn new(stream: DirRawStream<IO, TP, OCC>, fs: Rc<FileSystem<IO, TP, OCC>>) -> Self {
        Dir { stream, fs }
    }
...
impl<IO: ReadWriteSeek, TP: TimeProvider, OCC: OemCpConverter> Dir<IO, TP, OCC> {
    fn find_entry(
        &self,
        name: &str,
        is_dir: Option<bool>,
    ...
    pub fn find_volume_entry(&self) -> Result<Option<DirEntry<IO, TP, OCC>>, Error<IO::Error>> {
    ...
    pub fn create_dir(&self, path: &str) -> Result<Self, Error<IO::Error>> {
        ...
        match r {
            ...
            DirEntryOrShortName::ShortName(short_name) => {
                ...
                let cluster = self.fs.clone().alloc_cluster(None, true)?;
    ...
    pub fn remove(&self, path: &str) -> Result<(), Error<IO::Error>> {
        ...
        if let Some(n) = e.first_cluster() {
            self.fs.clone().free_cluster_chain(n)?;
        }
...
// Note: derive cannot be used because of invalid bounds. See: https://github.com/rust-lang/rust/issues/26925
impl<IO: ReadWriteSeek, TP: TimeProvider, OCC: OemCpConverter> Clone for Dir<IO, TP, OCC> {
    fn clone(&self) -> Self {
        Self {
...
pub struct DirIter<IO: ReadWriteSeek, TP, OCC> {
    stream: DirRawStream<IO, TP, OCC>,
    fs: Rc<FileSystem<IO, TP, OCC>>,
    skip_volume: bool,
    err: bool,
...
impl<IO: ReadWriteSeek, TP, OCC> DirIter<IO, TP, OCC> {
    fn new(stream: DirRawStream<IO, TP, OCC>, fs: Rc<FileSystem<IO, TP, OCC>>, skip_volume: bool) -> Self {
        DirIter {
            stream,
            fs,
            skip_volume,
...
impl<IO: ReadWriteSeek, TP: TimeProvider, OCC> DirIter<IO, TP, OCC> {
...
// Note: derive cannot be used because of invalid bounds. See: https://github.com/rust-lang/rust/issues/26925
impl<IO: ReadWriteSeek, TP, OCC> Clone for DirIter<IO, TP, OCC> {
    fn clone(&self) -> Self {
        Self {
            stream: self.stream.clone(),
            fs: self.fs.clone(),
...
impl<IO: ReadWriteSeek, TP: TimeProvider, OCC> Iterator for DirIter<IO, TP, OCC> {
